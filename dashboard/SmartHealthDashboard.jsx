...
const ALERTS = [
  ...
  { id: 5, time: "08:00", district: "Imphal East", type: "SENSOR", msg: "IoT sensor offline — Turbidity data unavailable since 07:30", severity: "medium" },
...
const ARCH_LAYERS = [
  { label: "Mobile App / SMS Gateway", color: "#185FA5", items: ["Flutter App", "IVRS SMS", "ASHA Forms", "Multilingual UI"] },
  { label: "Backend API Layer", color: "#0F6E56", items: ["Node.js REST API", "Auth & RBAC", "Alert Engine", "IoT Gateway"] },
...
export default function App() {
  ...
  const handlePredict = async () => {
    ...
    setPredError(null);
    try {
      const prompt = `You are an AI epidemiologist for the Smart Health Monitoring System for rural Northeast India.
...
Respond ONLY with a valid JSON object, no markdown, no extra text:
{
  ...
  "time_to_outbreak": "estimated days if no intervention or null"
      ...
      setPredError("Prediction failed: " + e.message);
    }
    setPredicting(false);
  };
  ...
  const tabs = [
    ...
    { id: "prediction", label: "AI Prediction Engine" },
        ...
        {tab === "overview" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12, marginBottom: 20 }}>
              {[
                { label: "Total Cases (NE India)", value: "588", sub: "2022 outbreak data", color: "#E24B4A" },
                { label: "Districts Monitored", value: "8", sub: "across 6 NE states", color: "#185FA5" },
                { label: "Active Outbreaks", value: "3", sub: "ADD, Cholera, Dengue", color: "#EF9F27" },
                { label: "High Risk Districts", value: "4", sub: "immediate action needed", color: "#A32D2D" },
                { label: "IoT Sensors Active", value: "24", sub: "water quality sensors", color: "#1D9E75" },
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div>
              <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "16px", marginBottom: 16 }}>
                <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 4, color: "var(--color-text-primary)" }}>AI/ML prediction engine — XGBoost + Claude AI</div>
                <div style={{ fontSize: 11, color: "var(--color-text-secondary)", marginBottom: 16 }}>Input real-time field data from ASHA workers and IoT sensors</div>
                ...
                <button onClick={handlePredict} disabled={predicting} style={{
                  ...
                  {predicting ? "Running AI prediction model..." : "Run Outbreak Prediction ↗"}
                </button>
              </div>
            ...
            <div>
              ...
              {predResult ? (
                ...
                <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: 40, textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 13, color: "var(--color-text-secondary)", maxWidth: 240, margin: "0 auto" }}>Adjust the input parameters and click "Run Outbreak Prediction" to get AI-powered risk analysis</div>
                </div>
              )}
    ...
    Model: LSTM (TensorFlow 2.21) • Sequence: 8 weeks • Forecast: 4 weeks • Features: 21
        ...
        {tab === "water" && (
          <div>
            ...
            {/* IoT Sensor Status */}
            <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "16px" }}>
              <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 12, color: "var(--color-text-primary)" }}>IoT water sensor network status</div>
